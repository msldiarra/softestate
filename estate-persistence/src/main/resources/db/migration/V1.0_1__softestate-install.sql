...
CREATE TABLE IF NOT EXISTS property_floor_media (
  id SERIAL PRIMARY KEY,
  property_id INTEGER NOT NULL,
...
--- todo rename to owner_company_name
...
CREATE VIEW Users AS
  SELECT c.id, c.first_name, c.last_name, ci.email, l.login, l.password, l.enabled, cu.name as customer
  FROM contact_login AS cl
    INNER JOIN login AS l ON l.Id = cl.login_id
    INNER JOIN contact AS c ON c.Id = cl.contact_id
    INNER JOIN contact_info AS ci ON ci.Id = c.contact_info_id
...
ALTER TABLE property_floor_media
ADD CONSTRAINT fk_property_floor_media_property FOREIGN KEY (property_id) REFERENCES property (id);
